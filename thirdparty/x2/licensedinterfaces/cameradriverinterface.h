...
public://Properties
	...
	virtual int			pathTo_rm_FitsOnDisk(char* lpszPath, const int& nPathMaxSize){(void)lpszPath; (void)nPathMaxSize;return 0;}
...
public://Methods
	...
	/*!Return the temperature and corresponding status.  Because camera hardware varies widely with respect to temperature control, this method is intended to accomodate 
	a wide range of levels of support, from the most basic, where only temperature regulation can be enabled/disabled to the most advanced that includes regulation enabled/disabled, 
	percent power, a discriptive string assocaited with power, and setpoint or any combination.
	\param dCurTemp Return the current temperature or -100 if not supported. 
	\param dCurPower Return the power applied to the TE cooler or -100 if not supported.
	\param lpszPower Return a string associated the power applied to the TE cooler or NULL if not supported.  Example strings might be "Off", "Cooling To Setpoint", 
	"Correcting", "Raising to Ambient", "Ambient", "Max", "Min", "OK (At Setpoint)" 
	\param nMaxLen The maximum size the lpszPower string can be.
	\param bCurEnabled Return 1 if temperature is being regulated or 0 if not.
	\param dCurSetPoint Return the current temperature setpoint or -100 if not supported.*/
