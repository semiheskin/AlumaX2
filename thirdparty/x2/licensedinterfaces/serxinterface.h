...
public:
	...
	This argument can also be used to set the DTR, for example "-DTR_CONTROL 1" will cause DTR to be on.
	*/
	...
	\param nNumber number of bytes.
	\param nTimeOutMilli timeout in to wait in milliseconds.
	*/
	virtual int waitForBytesRx(const int& nNumber, 
                             const int& nTimeOutMilli) = 0;

	/*! Read dwTot bytes from the receive port, or timeout.
	\param lpBuf pointer to the data.
	\param dwTot the total number of bytes to read.
	\param dwRed (sic) the total number of bytes actually read.
	...
	*/
	...
	\param lpBuf pointer to the data.
	\param dwTot the total number of bytes to write.
	\param dwRote (sic) the total number of bytes actually written.
	*/
	virtual int writeFile(void* lpBuf,
                        const unsigned long& dwTot, 
                        unsigned long& dwRote) = 0;
